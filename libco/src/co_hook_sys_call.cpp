...
ssize_t sendto(int socket,
    ...
    */
    ...
    if (ret < 0 && EAGAIN == errno) {
        ...
        ret = g_sys_sendto_func(socket, message, length, flags, dest_addr,
                                dest_len);
    }
    return ret;
}
...
ssize_t recvfrom(int socket,
    ...
    ssize_t ret = g_sys_recvfrom_func(socket, buffer, length, flags, address,
                                      address_len);
    return ret;
}
...
int poll(struct pollfd fds[], nfds_t nfds, int timeout) {
    ...
    return co_poll_inner(co_get_epoll_ct(), fds, nfds, timeout,
                         g_sys_poll_func);
}
...
int setenv(const char* n, const char* value, int overwrite) {
    ...
    if (co_is_enable_sys_hook() && g_co_sysenv.data) {
        ...
        if (self) {
            ...
            stCoSysEnv_t* e = (stCoSysEnv_t*)bsearch(
                &name, arr->data, arr->cnt, sizeof(name), co_sysenv_comp);
...
struct hostent* co_gethostbyname(const char* name) {
    ...
    while (ret = gethostbyname_r(name, host, __co_hostbuf_wrap->buffer,
        ...
        __co_hostbuf_wrap->buffer =
            (char*)malloc(__co_hostbuf_wrap->iBufferSize);
    }
